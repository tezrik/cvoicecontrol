...
int generateConfiguration(int autohome)
{
    ...
    mvwaddstrcntr(savescr, 6, width, CVC_CONFIG_HOMEPATHFILE);
    ...
    if (saveConfiguration(autohome) == 0)
    {
        if (autohome)
        {
            /***** clear dialog */
...
int main(int argc, char *argv[])
{
    ...
    while (!request_finish)
    {
        ...
        if (mixerOK() == MIXER_OK)
        {
            mvwaddstr(mainscr, 7,24," (");
            waddstr(mainscr, getMixer());
            waddstr(mainscr, ")");
        }
        else
        ...
        if (audioOK() == AUDIO_OK)
        {
            mvwaddstr(mainscr, 8,24," (");
            waddstr(mainscr, getAudio());
            waddstr(mainscr, ")");
        }
        else
        ...
        case BLANK:
            ...
            case 5: /***** save configuration! */
                if (generateConfiguration(autohome))
                {
                    status[5] = ok;
                    status[6] = ok;
                }
